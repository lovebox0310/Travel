...
<body>
	...
	<div class="container" style="margin: 30px auto 30px auto;">
		...
		<div class="dropdown">
			...
			<div class="dropdown-menu">
				<a class="dropdown-item" href="board7asklist.do?id=${param.id}">나의 질문 목록</a>
				<div class="dropdown-divider"></div>
				<a class="dropdown-item" href="board7replylist.do?id=${param.id}">나의 답변 목록</a>
			</div>
		</div>
		<br>
  ...
  <path fill-rule="evenodd" d="M3 2.5a.5.5 0 0 0-.5.5v4A2.5 2.5 0 0 0 5 9.5h8.5a.5.5 0 0 0 0-1H5A1.5 1.5 0 0 1 3.5 7V3a.5.5 0 0 0-.5-.5z" />
							...
							<a style="color: rgb(79, 79, 79);" href="board7read.do?num=${dto.num}&id=${param.id}">
		...
		<ul class="pagination justify-content-center">
			<li class="page-item ${to.curPage eq 1 ? 'disabled' : '' }"><a class="page-link"
					href="board7qnalist.do?curPage=${to.curPage ne 1 ? (to.curPage - 1) : 1 }&id=${param.id}&search=${search}&find=${find}">Previous</a></li>
			<c:forEach begin="${to.beginPageNum }" end="${to.stopPageNum }" var="idx">
				<li class="page-item ${idx eq to.curPage ? 'active' : '' }"><a class="page-link"
						href="board7qnalist.do?curPage=${idx }&id=${param.id}&search=${search}&find=${find}">${idx }</a></li>
			</c:forEach>
			<li class="page-item ${to.curPage eq to.totalPage ? 'disabled' : '' }"><a class="page-link"
					href="board7qnalist.do?curPage=${to.curPage ne to.totalPage ? to.curPage + 1 : to.totalPage }&id=${param.id}&search=${search}&find=${find}">Next</a></li>
		</ul>



		<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
			<form class="form-inline " name='sform' method='get' action='./board7qnalist.do'>
				...
				<aside>
					...
					<a class="btn btn-success btn-sm" href="board7qnalist.do?id=${param.id}">전체글 보기</a>
				</aside>
		...
		<br>
		<a style="position: relative; left: 90%" href="board7askui.do?id=${param.id}&writer=${writer}" class="btn btn-info">
			<strong>질문하기</strong>
		</a>
