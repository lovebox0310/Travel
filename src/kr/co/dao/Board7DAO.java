...
public class Board7DAO {
	...
	public PageTO page(String find, String search, int curPage) {
		...
		try {
			...
			while (rs.next()) {
				...
				Board7DTO dto = new Board7DTO(id, num, writer, title, null, writeday, readcnt, -1, -1, repIndent);
				list.add(dto);
			}
			to.setList(list);
	...
	public void insert(Board7DTO boardDTO) { // 글 작성
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public Board7DTO read(int number) {
		Board7DTO dto = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			if (rs.next()) {
				...
				dto = new Board7DTO(id, number, writer, title, content, writeday, readcnt, 0, 0, 0, filename);
				isOk = true;
			}
	...
	public Board7DTO updateUI(int number) {
		Board7DTO dto = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "select * from qnaboard where num = ?";
		...
		try {
			...
			if (rs.next()) {
				...
				dto = new Board7DTO(null, number, writer, title, content, null, 0, repRoot, repStep, repIndent);
	...
	public void update(Board7DTO dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public void reply(int orgnum, Board7DTO dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "insert into qnaboard (id, num,writer,title,content,repRoot,repStep,repIndent,filename) values (?,?,?,?,?,?,?,?,?)";
		...
		try {
			...
			int num = createNum(conn);
			Board7DTO orgDTO = updateUI(orgnum);
	...
	private void stepPlus(Connection conn, Board7DTO orgDTO) {
		PreparedStatement pstmt = null;
	...
	/**
	 * 
	 * 본인이 작성한글의 repRoot값 가져오기
	 * @param id
	 * @return
	 */
	public List<Board7DTO> getAskRepRoots(String id) {
		String sql = "select * from qnaboard where repRoot in (select repRoot from qnaboard where id = ? and repIndent = 0 ) order by repRoot desc, repStep asc"; 
																																									
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<Board7DTO> list = new ArrayList<Board7DTO>();
		...
		try {
			...
			while (rs.next()) {
				...
				int readcnt = rs.getInt("readcnt");
				int repIndent = rs.getInt("repIndent");
				Board7DTO dto = new Board7DTO(rid, num, writer, title, null, writeday, readcnt, -1, -1, repIndent);
				list.add(dto);
			}
	...
	public List<Board7DTO> getReplyRepRoots(String id) {
